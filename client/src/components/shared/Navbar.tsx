	...
	return (
		<div
			className={`w-full h-full flex justify-between items-center ${
				meetingHeadLine ? "px-3 sm:px-6" : "px-3 sm:px-8"
			}`}
		>
			{meetingHeadLine ? (
				meetingHeadLine
				...
				>
					<button type="button" className="mr-4 flex sm:hidden" onClick={toggleSidebar}>
						{isSidebarOpen ? XMark() : MenubarIcon()}
					</button>
					{activePage}
				</h1>
			)}
