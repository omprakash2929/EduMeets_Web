	...
	return (
		...
		>
			...
			{error ? (
				<MessageDisplay
					hasBackground={true}
					message={errorFormat(error)}
					styles={{ height: "calc(100% - 92px)", width: "calc(100% - 32px)", margin: "16px" }}
				/>
			) : (
				<div className={`relative flex w-full`} style={{ height: "calc(100% - 60px)" }}>
					<div className="flex flex-col flex-1 h-full">
						...
						>
							<SelfStream startTime={startTime} elapsedTime={elapsedTime} isLoading={isLoading} />
							{renderPeersVidoes.length > 0 && <PeersStream />}
						</div>
						{isLoading ? (
							<MessageDisplay height={"h-[70px]"} hasBackground={true} />
						) : (
							<Preferences
								startTime={startTime}
